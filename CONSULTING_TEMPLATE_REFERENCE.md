# PPTagent — 四大咨询风格参考（仅作灵感）

> ⚠️ **本文不再作为验收标准**。验收标准见 [`REQUIREMENTS.md`](./REQUIREMENTS.md)。
>
> **2026-04-30 校准**：经需求对齐，PPTagent 的对标定位为**国内 B2B 售前方案 deck**（华为白皮书 / 阿里云解决方案 / Gartner 报告 / 罗兰贝格 briefing 风格），**不再对标 MBB 极简战略 deck**。本文以下章节中与该定位冲突的部分（white space / minimalism / 单 accent 颜色严格化 / bullet 30-60 字等）已**作废**，详见 §九 与 `REQUIREMENTS.md`。
>
> **本文剩余价值**：
> - §一 真实素材库（公开下载链接）— 仍可作灵感参考
> - §二 §1 §2 §5 §7（action title / pyramid / 图表注释 / MECE）— 售前 deck 通用，仍有效
> - §一 林林总总的 MBB 报告链接 — 可作为图表呈现、章节扉页设计的视觉灵感
>
> **不再使用本文做的事**：
> - 第三部分 13 项 gap 评分（用 `REQUIREMENTS.md` §七 audit 表替代）
> - 第五部分 MBB Rubric（用 `REQUIREMENTS.md` §二 SLO 替代）
> - 第四部分 Fix 7-13 spec 中与 minimalist 假设绑定的项
...
> ❌ **以下"2-3 色封顶 / 单 accent 严格化"规则已作废**。售前 deck 允许 3+ 色（章节色等）。视觉灵感参考保留，但请按 `REQUIREMENTS.md` §3.2 验收。
...
> ❌ **本节作废**——售前 deck 风格反向。新规则见 `REQUIREMENTS.md` §3.1：
> - 文字像素覆盖：**50-75%**（不是 < 60%）
> - 单页论据数：**6-8**（不是 3-4）
> - Bullet 字数：**60-120 字**（不是 30-60 字）
> - 留白节制使用，不留大片空白
...
> ⚠️ **本表部分作废**。基于 MBB 极简假设的项（G3 chart 数量目标 30-50%、G6 bullet 长度 30-60 字、G9 white space）已不再有效。当前 audit 见 `REQUIREMENTS.md` §七。
...
> ❌ **本节整体作废**。MBB Rubric 10 维度评分基于极简风假设（C. White space、E. Color semantic 等）与售前 deck 需求冲突。release 评测改用 `REQUIREMENTS.md` §二 SLO（H1-H5 Hard + S1-S5 Soft）。
>
> 以下 10 维度仅作历史记录：
