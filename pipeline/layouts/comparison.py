...
class ComparisonLayout:
    ...
    def build_html(self, content: ComparisonContent, theme_colors: dict,
        ...
        return (
            ...
            '\n'
            f'<div style="position:absolute; left:480px; top:72px; width:1px; height:434px; '
            'background-color:#E0E0E0;"></div>\n'
            '\n'
            f'<div style="position:absolute; left:40px; top:76px; width:420px; height:28px; '
            f'background-color:{bg};">\n'
            f'  <p style="font-size:12px; font-weight:bold; color:{primary}; '
            f'margin:5px 8px;">{_html.escape(content.left_label)}</p>\n'
            '</div>\n'
            f'<div style="position:absolute; left:40px; top:112px; width:420px; height:398px; '
            'overflow:hidden;">\n'
            f'{_bullets(content.left_bullets)}\n'
            '</div>\n'
            '\n'
            f'<div style="position:absolute; left:496px; top:76px; width:420px; height:28px; '
            f'background-color:{bg};">\n'
            f'  <p style="font-size:12px; font-weight:bold; color:{primary}; '
            f'margin:5px 8px;">{_html.escape(content.right_label)}</p>\n'
            '</div>\n'
            f'<div style="position:absolute; left:496px; top:112px; width:420px; height:398px; '
            'overflow:hidden;">\n'
